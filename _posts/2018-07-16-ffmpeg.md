...
**给视频加上当前时间水印**
```
ffmpeg -re -i sample.flv -vf "settb=AVTB,setpts='trunc(PTS/1K)*1K+st(1,trunc(RTCTIME/1K))-1K*trunc(ld(1)/1K)',drawtext=fontsize=100:fontcolor=white:text='%{localtime}.%{eif\:1M*t-1K*trunc(t*1K)\:d}'" -c:a libx264 -c:a copy -f flv "rtmp://127.0.0.1/live/test"
```

**循环推流**
```
ffmpeg -threads 2 -re -fflags +genpts -stream_loop -1 -i sample.flv -c copy -f flv "rtmp://127.0.0.1/live/test"
```