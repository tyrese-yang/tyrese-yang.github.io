...
“`<>`”中的值定义在f4v中，“`[abstURL]`”是f4m中abst标签的url属性值， “`[mediaURL]`”是f4m中media标签的url属性值。如果`<ServerEntryCount>`为0，`<ServerBaseUrl>`和末尾的斜杆应该被省略，使用f4m所在地址(路径不包括f4m文件名)作为ServerBaseUrl。
...
Bootstrap Info，切片列表信息，定义在f4v协议中的box类型，记录切片的时间戳、序号和时长等信息，列表信息会随着直播的进行不断更新，播放器可以根据列表中的序号去请求对应的切片。abst中包含asrt、afrt，协议中SegmentRunTableEntries指的就是`asrt`，可以有多个；FragmentRunTableEntries是`afrt`，也可以有多个。

### asrt
asrt用来定位Segment。segment的序号从asrt的SegmentRunEntryTable中计算出来。
SegmentRunEntryTable是一个数组，每个元素有两个字段。  
...
| FirstFragmentTimestamp | UI64 | 时间戳 |
| FragmentDuration | UI32 | 时长 |
| DiscontinuityIndicator | IF FragmentDuration == 0 UI8 | 不连续描述符，具体见协议 |
